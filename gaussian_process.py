...
X_locf_og = df_locf[all_features]
y_locf_og = df_locf['goal']

X_mean_og = df_mean[all_features]
y_mean_og = df_mean['goal']
...
print(X_locf_oversampled.columns)

X_locf_oversampled_temp = X_locf_oversampled[:1000].copy()
y_locf_oversampled_temp = y_locf_oversampled[:1000].copy()

X_mean_oversampled_temp = X_mean_oversampled[:1000].copy()
y_mean_oversampled_temp = y_mean_oversampled[:1000].copy()

gpc_locf_oversampled = GaussianProcessClassifier(kernel=kernel, random_state=0, n_jobs=-1, multi_class='one_vs_one')
gpc_locf_oversampled.fit(X_locf_oversampled_temp, y_locf_oversampled_temp)

gpc_mean_oversampled = GaussianProcessClassifier(kernel=kernel, random_state=0, n_jobs=-1, multi_class='one_vs_one')
gpc_mean_oversampled.fit(X_mean_oversampled_temp, y_mean_oversampled_temp)

y_locf_oversampled_pred = gpc_locf_oversampled.predict(X_locf_oversampled)
y_mean_oversampled_pred = gpc_mean_oversampled.predict(X_mean_oversampled)

y_locf_oversampled_prob = gpc_locf_oversampled.predict_proba(X_locf_oversampled)
y_mean_oversampled_prob = gpc_mean_oversampled.predict_proba(X_mean_oversampled)
